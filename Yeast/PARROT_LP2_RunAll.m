...
for k = 1:numel(growth_conditions)
    ...
    for lambda = lambdaVals
        ...
        %Calculate Pearson correlatiom
        LPsolution.x = LPsolution.full(length(modelREF.rxns)+1:end);
        enzymeLevels = LPsolution.x(enzymeIds);
    ...
    % title('Effect of lambda on Pearson correlation');
    
    % Solve the problem again with optimal lambda
    modelDelta.c(1:rxns_index) = optimalLambda*NormMinSol.full(1:rxns_index);
