...
for k = 1:numel(growth_conditions)
    ...
    % Loop over lambda values to find the maximum Pearson correlation
    maxCorrelation = 0;
    optimalLambda = 0;
    corrVals = [];
    ...
    for lambda = lambdaVals
        ...
        enzymeLevels = QPsolution.full(enzymeIds);
    
        pred = {};
        pred(:,1) = modelSTR.rxns(enzymeIds(1:end-1));
        pred(:,1) = replace(pred(:,1), 'draw_prot_', '');
        pred(:,2) = num2cell(enzymeLevels(1:end-1));
        ...
        mergedLog.Predicted(mergedLog.Predicted==-Inf) = 0;
        mergedLog.Abundance(mergedLog.Abundance==-Inf) = 0;
    
        correlation = corr(mergedLog.Predicted, mergedLog.Abundance, 'type', 'Pearson');
        corrVals = [corrVals, correlation];
    
        % Update optimal lambda if correlation improves
    ...
    corrVals_LAHTVEE(k,:) = corrVals;

    % Plot the correlation values as a function of lambda
    % plot(lambdaVals, corrVals);
    % xlabel('Lambda values');
    % ylabel('Pearson correlation');
    % title_fig = "Effect of lambda on Pearson correlation - " + current_condition;
    % title(title_fig);

    % fprintf('\n' + "Optimal lambda: " + optimalLambda + '\n');

    % Solve the problem again with optimal lambda
    QPproblem.c(rxnIds) = -2*optimalLambda*QPproblem.c(rxnIds);
    QPproblem.F(1:nRxnsSTR,1:nRxnsSTR) = 2*optimalLambda*speye(nRxnsSTR);
    ...
    results_LAHTVEE.OptimalLambda(k) = optimalLambda;
...
% Define the names of the growth conditions
growth_conditions = constraints.Properties.VariableNames(10:16);
...
for k = 1:numel(growth_conditions)
    ...
    modelSTR = FlexibilizeConstraints(modelSTR, 1.5);

    % fix specific growth rate at the dilution rate, allowing 5% flexibility
    modelSTR = changeRxnBounds(modelSTR, 'r_2111', flux_values(1)*(1+0.9), 'u');
    modelSTR = changeRxnBounds(modelSTR, 'r_2111', flux_values(1)*(1-0.9), 'l');   
    ...
    corrVals_YU2021_N30(k,:) = corrVals;
    ...
    results_YU2021_N30.OptimalLambda(k) = optimalLambda;
...
for k = 1:numel(growth_conditions)
    ...
    corrVals_YU2021_Gln(k,:) = corrVals;
    ...
    results_YU2021_Gln.OptimalLambda(k) = optimalLambda;
...
for k = 1:numel(growth_conditions)
    ...
    corrVals_YU2021_Phe(k,:) = corrVals;
    ...
    results_YU2021_Phe.OptimalLambda(k) = optimalLambda;
...
for k = 1:numel(growth_conditions)
    ...
    corrVals_YU2021_Ile(k,:) = corrVals;
    ...
    results_YU2021_Ile.OptimalLambda(k) = optimalLambda;
...
for k = 1:numel(growth_conditions)
    ...
    results_YU2020.OptimalLambda(k) = optimalLambda;
